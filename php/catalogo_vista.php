...
<form action="productos.php" method="get" class="row g-3 mb-4 catalog-filters" role="search">
    ...
    <label class="col-md-6">Categoría<select name="categoria" class="form-select"><option value="">Todas las categorías</option>
        <?php foreach (navigation_categories() as $category): ?><option value="<?= $category['id_categoria'] ?>" <?= (string) $category['id_categoria'] === $filters['categoria'] ? 'selected' : '' ?>><?= h($category['nombre_categoria']) ?></option><?php endforeach; ?>
    </select></label>
    ...
    <label class="col-lg-6">Ordenar por<select name="orden" class="form-select">
        <?php foreach (['recientes' => 'Más recientes', 'precio_asc' => 'Precio: menor a mayor', 'precio_desc' => 'Precio: mayor a menor', 'nombre' => 'Nombre: A–Z'] as $value => $label): ?><option value="<?= $value ?>" <?= $filters['orden'] === $value ? 'selected' : '' ?>><?= $label ?></option><?php endforeach; ?>
    </select></label>
    <div class="col-12 d-flex align-items-center flex-wrap gap-3">
        <label class="d-flex align-items-center gap-2"><input name="stock" type="checkbox" value="1" <?= $filters['stock'] ? 'checked' : '' ?>>Solo disponibles</label>
        <button class="btn action" type="submit">Buscar</button><a class="btn secondary" href="productos.php">Limpiar filtros</a>
    </div>
</form>
